...
def format_description(species, trim_type, width, thickness, finish_level, is_finishing, finish_type):
    """Format description: Species PRIMED/FINISHED TYPE Width x Thickness"""
    # Determine if PRIMED or FINISHED
    if is_finishing:
        finish_label = finish_type.upper() if finish_type else 'FINISHED'
    else:
        finish_label = 'PRIMED'
    ...
    type_map = {
        ...
        'sills': 'SILL',
        'apron': 'APRON',
        'jambs': 'INT JAMB',  # Will need to distinguish window jamb
        'dentils': 'DENTIL'
    }
    type_name = type_map.get(trim_type, trim_type.upper())
    
    return f"{species} {finish_label} {type_name} {width} x {thickness}"
...
def generate_invoice_excel(all_results, species, lf_cost_per_ft, bf_markup_pct, is_finishing, finish_type, 
            ...
            if trim_type in pricing:
                ...
                if p['linear_ft'] > 0:
                    ...
                    line_items.append({
                        ...
                        'lf_price': p['base_price_per_lf'],
                        'setup': p['setup_cost'],
                        'finish': p['finish_multiplier'],
                        'lf_price_final': p['price_per_lf'],
    ...
    total_subtotal = sum(item['line_total'] for item in line_items)
    ws['G31'] = species
    ...
    # Row 34: Total
    tax_amount = total_subtotal * sales_tax_rate
    total_with_tax = total_subtotal + tax_amount
    ws['G34'] = 'Total'
