...
from config_validator import validate_config_files
...
class TrimCalculatorApp:
    ...
    def create_form_fields(self, parent):
        ...
        # Manual Trim Style dropdown
        manual_row = 0
        ttk.Label(self.manual_lf_frame, text="Manual Trim Style:").grid(
            row=manual_row, column=0, sticky=tk.W, padx=5, pady=5)
        style_labels = {
            'craftsman': 'Craftsman',
            'mitered': 'Mitered',
            'built_up': 'Built-Up',
        ...
        style_values = [style_labels.get(style, style.replace('_', ' ').title()) for style in TRIM_STYLES]
        self.inputs['manual_trim_style'] = ttk.Combobox(self.manual_lf_frame, values=style_values, width=17, state="readonly")
        self.inputs['manual_trim_style'].set('Craftsman')  # Default to Craftsman
        self.inputs['manual_trim_style'].grid(row=manual_row, column=1, sticky=(tk.W, tk.E), padx=5, pady=5)
        self.inputs['manual_trim_style'].state(['disabled'])  # Disabled by default
        manual_row += 1
    ...
    def toggle_lf_input_mode(self):
        ...
        if self.inputs['has_lf_takeoffs'].get():
            ...
            # Enable the manual trim style dropdown
            self.inputs['manual_trim_style'].state(['!disabled'])
        else:
            # Show standard inputs, hide manual LF inputs
            self.manual_lf_frame.grid_remove()
            self.standard_input_frame.grid()
            # Disable the manual trim style dropdown
            self.inputs['manual_trim_style'].state(['disabled'])
    ...
    def validate_configuration(self):
        """Validate configuration files on startup"""
        is_valid, errors, warnings = validate_config_files(show_dialogs=True)
    ...
    def calculate(self):
        ...
        if not self.config_valid:
            if not self.validate_configuration():
                messagebox.showerror("Configuration Error", 
                                   "Cannot calculate: Configuration files are invalid.\n"
        ...
        if inputs.get('has_lf_takeoffs', False):
            ...
            # Get selected manual trim style from dropdown
            manual_trim_style_display = self.inputs['manual_trim_style'].get()
            # Map display name back to internal style name
            style_labels = {
                'Craftsman': 'craftsman',
                'Mitered': 'mitered',
                'Built-Up': 'built_up',
                'Sill/Apron Only': 'sill_apron_only'
            }
            manual_trim_style = style_labels.get(manual_trim_style_display, 'craftsman')
            
            # Fallback: if style is missing or unreadable, default to Craftsman
            if manual_trim_style not in TRIM_STYLES:
                import logging
                logging.warning(f"Invalid trim style '{manual_trim_style}', defaulting to 'craftsman'")
                manual_trim_style = 'craftsman'
            
            for style in TRIM_STYLES:
                # Use manual LF values
                lf_results = manual_lf.copy()
                
                # Calculate BF using the selected manual trim style for dimensions
                bf_data = calculate_bf_with_waste(lf_results, manual_trim_style, finish_level)
    ...
    def display_results(self, style, lf_results, bf_data, inputs):
        ...
        # For manual LF mode, show the trim style used for BF calculations
        applied_style_label = style_label
        if inputs.get('has_lf_takeoffs', False):
            manual_trim_style_display = self.inputs['manual_trim_style'].get()
            applied_style_label = manual_trim_style_display
        
        output = f"Trim Style: {style_label}\n"
        output += f"Finish Level: {inputs['finish_level']}\n"
        output += f"Door Height: {inputs['door_height']}\n"
        if inputs.get('has_lf_takeoffs', False):
            output += f"BF Calculation Style: {applied_style_label}\n"
        output += "=" * 70 + "\n\n"
        ...
        for trim_type in trim_type_order:
            if trim_type in lf_results:
                linear_ft = round(lf_results[trim_type], 2)
                bf_required = round(bf_data['bf_with_waste'].get(trim_type, 0.0), 2)
                ...
                if linear_ft > 0 or trim_type in ['base', 'casing']:  # Always show base and casing
                    ...
                    # Improved label format: "Base – 128.50 LF, 42.37 BF (Craftsman)"
                    trim_label = trim_type.replace('_', ' ').title()
                    if inputs.get('has_lf_takeoffs', False):
                        style_info = f" ({applied_style_label})"
                    else:
                        style_info = f" ({style_label})"
                    output += f"{trim_label:<20} {linear_ft:>12.2f} {bf_required:>15.2f} {thickness_str:>20} {width_str:>12}\n"
        
        output += "-" * 90 + "\n"
        
        # Totals
        total_lf = round(sum(lf for lf in lf_results.values() if lf > 0), 2)
        total_bf_required = round(sum(bf for bf in bf_data['bf_with_waste'].values()), 2)
