...
def validate_trim_dimensions(script_dir):
...
def validate_awi_waste_chart(script_dir):
...
def validate_lumber_species(script_dir):
...
def validate_bf_cost(script_dir):
...
def validate_finish_rates(script_dir):
    """Validate Finish Rates.xlsx schema (if exists)"""
    excel_path = os.path.join(script_dir, 'Finish Rates.xlsx')
...
def validate_setup_costs(script_dir):
    """Validate setup_costs.xlsx schema (if exists)"""
    excel_path = os.path.join(script_dir, 'setup_costs.xlsx')
...
def validate_finish_pricing(script_dir):
    """Validate finish_pricing.xlsx schema (if exists)"""
    excel_path = os.path.join(script_dir, 'finish_pricing.xlsx')
...
def validate_config_files(show_dialogs=True):
    ...
    optional_validations = [
        ('Finish Rates.xlsx', validate_finish_rates),
    ...
    for file_name, validation_func in validations:
        is_valid, message = validation_func(script_dir)
        if not is_valid:
    ...
    for file_name, validation_func in optional_validations:
        result, message = validation_func(script_dir)
