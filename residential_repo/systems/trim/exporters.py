"""
Trim system module: Export trim results into invoices/quotes.
...
def load_bf_cost():
    ...
    # TODO: Update path to use config/ directory
    script_dir = os.path.dirname(os.path.abspath(__file__))
    excel_path = os.path.join(script_dir, '..', '..', 'config', 'bf_cost.xlsx')
...
def setup_output_folders(base_dir=None):
    """Create Output and archive folders if they don't exist"""
    if base_dir is None:
        base_dir = os.path.join(os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__)))), 'data_samples', 'trim')
...
def generate_invoice_excel(all_results, species, lf_cost_per_ft, bf_markup_pct, is_finishing, finish_type, 
    ...
    Args:
        ...
        output_path: Path to save Excel file (if None, uses data_samples/trim/output folder)
    """
    # Setup output folders
    output_dir, archive_dir = setup_output_folders()
